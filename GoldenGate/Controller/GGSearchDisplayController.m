...
- (GGSegmentedControl*)searchScopeSegmentedControl
{
    ...
    {
        _searchScopeSegmentedControl = [[GGSegmentedControl alloc]initWithItems:@[NSLocalizedString(@"ChannelsLKey",@"") ,NSLocalizedString(@"VideosLKey", @"")]];
...
- (void)executeFullSearch
{
    ...
    self.fullSearchOperation = [NSBlockOperation blockOperationWithBlock:^
    {
        ...
        (SearchResultViewController *)self.searchContentsController.navigationController.topViewController;
        if ([searchResultController isKindOfClass:[SearchResultViewController class]])
        {
            [self dismissPopover];
            [self hideTapToDismissView];
        }
