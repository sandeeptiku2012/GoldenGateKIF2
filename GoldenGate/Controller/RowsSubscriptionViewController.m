...
- (void)viewDidLoad
{
    ...
    NSArray *centerNavBarViews = [self.navigationItem.titleView subviews];
    
    NSArray *leftBarButtonItems = self.navigationItem.leftBarButtonItems;
    for (int i=0; i<[leftBarButtonItems count]; i++) {
        if (i == 0) {
            UIBarButtonItem *barButtonItem = [leftBarButtonItems objectAtIndex:i];
            barButtonItem.accessibilityLabel = @"SubscriptionsBtn";
            break;
        }
    }
}
