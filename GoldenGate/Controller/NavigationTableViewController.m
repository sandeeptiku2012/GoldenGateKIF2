...
{
    // Hide first section header as in the designs.
    if (section == 0)
    {
        return @"";
    }

    return [self.sectionNames objectAtIndex:(NSUInteger) section];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell)
    {
        ...
        //        cell.textLabel.font = [[GGLabelStylizer sharedInstance]stylizedFontFromSystemFont:[UIFont systemFontOfSize:kTableCellLabelFontSize]];
    }
    ...
    {
        CategoryAction *currentCategoryAction = (CategoryAction *) self.mainViewController.navAction.parentAction;
    ...
    if ([cell.textLabel.text isEqualToString:@"Subscriptions"])
        cell.accessibilityLabel = @"SubscriptionsCell";
    cell.selectionStyle = UITableViewCellSelectionStyleGray;
    
    BOOL shouldShowRowImage = indexPath.section == 0;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([action isKindOfClass:[CategoryAction class]])
    {
        ...
        // TODO: Add turning the disclosure button into a spinner that spins until we're done navigating.
        [CategoryNavigator navigateToCategoryWithId:(NSUInteger) categoryAction.category.identifier
                                 fromViewController:self.mainViewController completionHandler:^
        {
            // TODO: Add stopping of the loading spinner
        }];
    }
    else
    {
        [NavActionExecutor executeNavAction:action fromViewController:self.mainViewController];
