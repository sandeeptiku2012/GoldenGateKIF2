...
{
    User *user = _sessionManager.currentUser;
    RestSearchCategoryList *res = [_userContentClient getAccessibleCategoriesForUser:user.identifier platform:_platform sort:nil start:nil size:nil query:nil error:error];//@"level:SUB_SHOW"
    NSMutableArray *result = [NSMutableArray array];
    for (RestSearchCategory *category in res.categories)
    {
